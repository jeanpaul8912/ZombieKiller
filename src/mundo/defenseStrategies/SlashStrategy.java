...
public class SlashStrategy extends DefenseStrategy {
	...
	public void executeAttack() {
		...
		} else if (interfaz.getCampo().getPersonaje().getPrincipal().getMunicion() == 0)
	...
	public boolean slash () {
		...
		if (interfaz.getCampo().getJefe() != null) {
			if (interfaz.getCampo().getJefe().getEstadoActual().equals(Enemigo.ATACANDO) && interfaz.getCampo().getJefe().comprobarDisparo(xPosition, yPosition, Cuchillo.DANIO)) {
				interfaz.getCampo().getPersonaje().setEnsangrentado(false);
				interfaz.getCampo().getPersonaje().getCuchillo().setEstado(Arma.CARGANDO);
				seEncontro = true;
